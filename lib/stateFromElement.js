...
var BlockGenerator = function () {
  ...
  _createClass(BlockGenerator, [{
    ...
    value: function processBlockElement(element) {
      ...
      var allowRender = !SPECIAL_ELEMENTS.hasOwnProperty(tagName) || INLINE_ELEMENTS.hasOwnProperty(tagName);
      var block = {
        tagName: tagName,
        textFragments: [],
        type: type,
    ...
    value: function processInlineElement(element) {
      ...
      var isSelfClosing = element.isSelfClosing;
      ...
      if (customInlineFn) {
        var customInline = customInlineFn(element);
        if (customInline != null) {
          ...
          var mutability = customInline.mutability || 'IMMUTABLE';
          entityKey = _draftJs.Entity.create(customInline.type, mutability, customInline.data);
        }
      }
